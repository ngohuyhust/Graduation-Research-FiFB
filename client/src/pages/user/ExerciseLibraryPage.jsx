    ...
    <>
      ...
      <form className="panel mb-6 grid gap-3 md:grid-cols-5" onSubmit={submitFilters}>
        <div className="relative md:col-span-1">
          ...
          <input className="input rounded-full pl-10" placeholder="Search" value={filters.keyword} onChange={(event) => setFilters({ ...filters, keyword: event.target.value })} />
        </div>
        ...
        <>
          {rows.length ? (
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
              {rows.map((row) => (
                <article className="group overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-soft transition-all duration-200 hover:-translate-y-1 hover:shadow-card" key={row.id}>
                  <Link className="relative block bg-slate-100" to={`/exercises/${row.id}`}>
                    {row.gifUrl || row.gif_url ? <img className="aspect-square w-full object-contain p-4 transition-transform duration-300 group-hover:scale-105" src={row.gifUrl || row.gif_url} alt={row.name} /> : <div className="grid aspect-square place-items-center text-sm text-slate-400">No media</div>}
                    <span className="absolute inset-x-4 bottom-4 rounded-lg bg-ink/80 px-3 py-2 text-center text-xs font-semibold text-white opacity-0 backdrop-blur transition group-hover:opacity-100">View detail</span>
                  </Link>
                  <div className="space-y-3 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <Link className="font-semibold text-ink hover:text-mint" to={`/exercises/${row.id}`}>{row.name}</Link>
                      {isAuthenticated && <button className="btn-secondary px-3" title="Favorite" type="button" onClick={() => favorite(row.id)}><Heart size={16} /></button>}
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {(row.bodyParts || row.body_parts || []).slice(0, 3).map((x) => <span className="tag" key={x.id || x.name || x}>{x.name || x}</span>)}
                    </div>
                    <p className="text-xs text-slate-500">{(row.equipments || []).map((x) => x.name || x).join(", ") || "Bodyweight"}</p>
                  </div>
                </article>
              ))}
            </div>
