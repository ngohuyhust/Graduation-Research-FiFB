...
export default function ProfilePage() {
  ...
  const profileForm = useForm({
    defaultValues: { fullName: "", phone: "", avatarUrl: "", fitnessGoal: "", experienceLevel: "" },
  });
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
        <form className="panel space-y-4" onSubmit={profileForm.handleSubmit(saveProfile)}>
          ...
          <FormField label="Fitness goal">
            <textarea className="input min-h-28" {...profileForm.register("fitnessGoal")} />
          </FormField>
          <FormField label="Experience level">
            <div className="grid gap-2 sm:grid-cols-3">
              {["beginner", "intermediate", "advanced"].map((level) => (
                <label
                  className="cursor-pointer rounded-xl border border-slate-200 px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500 transition-all has-[:checked]:border-mint has-[:checked]:bg-mint/5 has-[:checked]:text-mint"
                  key={level}
                >
