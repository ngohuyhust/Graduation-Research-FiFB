...
export default function TrainerCertificatesPage() {
  ...
  return (
    <>
      <PageHeader title="Trainer Certificates" actions={<button className="btn-primary" type="button" onClick={() => setShowForm(!showForm)}><Plus size={17} /> Submit certificate <ChevronDown className={showForm ? "rotate-180" : ""} size={16} /></button>} />
      {showForm && (
            ...
            <article className="panel-hover" key={row.id || row.title}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex gap-3">
                  <div className="grid h-10 w-10 place-items-center rounded-xl bg-amber-50 text-amber-500"><Award size={20} /></div>
                  <div>
              ...
              {(row.rejectionReason || row.rejection_reason) && <p className="mt-3 text-sm text-red-600">{row.rejectionReason || row.rejection_reason}</p>}
            </article>
          ))}
        </div>
