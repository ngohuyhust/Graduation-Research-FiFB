...
describe("Nest workout sessions", () => {
  ...
  beforeAll(async () => {
    app = await createApp();
    repo = app.locals.nest.get(WorkoutSessionsRepository);
  });
  afterAll(async () => {
  ...
  beforeEach(() => {
    ...
    for (const [name, result] of Object.entries({
      ...
      update: { id },
      addLog: { id },
      ensureActiveExercise: true,
      list: { rows: [{ id }], total: 1 },
      progression: { exercise: { id, name: "Squat" }, progression: [], personalRecord: null },
      stats: {
        summary: { total_sessions: 1, this_week_sessions: 1, total_volume_kg: 100 },
        dates: [],
        weekly: [],
        topExercises: [],
      },
    }))
      jest.spyOn(repo, name).mockResolvedValue(result);
  });
  ...
  test("create and update use actor identity and invalidate statistics", async () => {
    const created = await request(app)
      ...
      .send({ title: " Push ", workoutPlanId: id, userId: id })
      .expect(201);
    ...
    await request(app)
      ...
      .set("Authorization", auth(actor))
      .send({ notes: null, completed: true })
      .expect(200);
  ...
  test("logging preserves zero weight and defaults set number", async () => {
    const res = await request(app)
      ...
      .set("Authorization", auth(actor))
      .send({ exerciseId: id, actualWeightKg: 0 })
      .expect(201);
  ...
  test("missing owned plan/session and inactive exercise prevent writes", async () => {
    ...
    await request(app)
      .post("/api/workout-sessions")
      .set("Authorization", auth(actor))
      .send({ title: "Push", workoutPlanId: id })
      .expect(404);
    ...
    await request(app)
      .post(`/api/workout-sessions/${id}/logs`)
      .set("Authorization", auth(actor))
      .send({ exerciseId: id, actualReps: 5 })
      .expect(400);
    expect(repo.addLog).not.toHaveBeenCalled();
    repo.ensureActiveExercise.mockResolvedValue(true);
    repo.addLog.mockResolvedValue(null);
    await request(app)
      .post(`/api/workout-sessions/${id}/logs`)
      .set("Authorization", auth(actor))
      .send({ exerciseId: id, actualReps: 5 })
  ...
  test("static stats route, cache TTL/hit and progression response remain intact", async () => {
    ...
    const progression = await request(app)
      .get(`/api/workout-sessions/progression/${id}`)
      .set("Authorization", auth(actor))
      .expect(200);
    expect(progression.body.data).toEqual({
      exerciseId: id,
      exerciseName: "Squat",
      progression: [],
      personalRecord: null,
