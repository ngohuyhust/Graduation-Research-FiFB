...
describe("Nest audit module behind admin endpoints", () => {
  ...
  beforeEach(() => {
    ...
    query = jest.spyOn(app.locals.nest.get(DatabaseService), "query").mockImplementation(async (sql) => ({ rows: sql.includes("count(*)") ? [{ total: 1 }] : [{ id, action: "trainer.verify" }] }));
  });
  afterEach(() => jest.restoreAllMocks());
  const auth = (actor) => `Bearer ${signAccessToken(actor)}`;
  test("filters and pagination use bound SQL parameters", async () => {
    const result = await request(app).get(`/api/admin/audit-logs?action=trainer.verify&entityType=trainer&actorId=${id}&page=2&limit=5`).set("Authorization", auth(actor)).expect(200);
    expect(result.body.data).toEqual({ items: [{ id, action: "trainer.verify" }], page: 2, limit: 5, total: 1, totalPages: 1 });
    expect(query).toHaveBeenLastCalledWith(expect.stringContaining("actor_id = $3"), ["trainer.verify", "trainer", id, 5, 5]);
  });
  test("audit insertion preserves caller transaction and metadata", async () => {
    const client = { query: jest.fn() };
    await app.locals.nest.get(AuditRepository).createAudit(client, { actorId: id, action: "user.locked", entityType: "user", entityId: id, oldValues: { status: "active" }, newValues: { status: "locked" }, ipAddress: "127.0.0.1", userAgent: "test" });
    expect(client.query).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO audit_logs"), [id, "user.locked", "user", id, { status: "active" }, { status: "locked" }, "127.0.0.1", "test"]);
