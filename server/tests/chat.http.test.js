...
describe("Nest chat HTTP and injected Socket.IO service", () => {
  ...
  beforeAll(async () => {
    app = await createApp();
    repo = app.locals.nest.get(ChatRepository);
  });
  afterAll(async () => {
  ...
  beforeEach(() => {
    ...
    for (const [method, result] of Object.entries({
      findMembership: { id, user_id: userId },
      listMessages: { rows: [{ id, content: "Hello" }], total: 1 },
      createMessage: { id, content: "Hello" },
      markRead: 2,
      unreadCounts: [{ connection_id: id, unread_count: 2 }],
    }))
      jest.spyOn(repo, method).mockResolvedValue(result);
  ...
  test("HTTP messages, read receipts and unread counts keep response and broadcasts", async () => {
    ...
    const message = await request(app)
      ...
      .send({ content: " Hello ", senderId: id })
      .expect(201);
    expect(message.body).toEqual({
      success: true,
      data: { message: { id, content: "Hello" } },
      message: "Message sent",
    });
    expect(repo.createMessage).toHaveBeenCalledWith(id, userId, { content: "Hello", messageType: "text" });
    expect(io.to).toHaveBeenCalledWith(`chat:${id}`);
    expect(io.emit).toHaveBeenCalledWith("chat:receive", { id, content: "Hello" });
  ...
  test("non-member cannot read, send or mark another conversation read", async () => {
    ...
    for (const [method, path] of [
      ...
      await request(app)
        [method](`/api/chat/${id}/${path}`)
        .set("Authorization", auth(actor))
        .send(method === "post" ? { content: "Hello" } : undefined)
        .expect(403);
  ...
  test.each(["chat:join", "chat:send", "chat:typing", "chat:read"])(
    "socket %s rejects non-members and malformed IDs",
    async (event) => {
      const ack = jest.fn();
