...
describe("Nest email deliveries with mocked provider", () => {
  ...
  const payload = { to: "recipient@example.com", subject: "Verify", html: "<p>Code</p>", templateKey: "email_verification" };
  beforeAll(async () => { app = await createApp(); repository = app.locals.nest.get(EmailDeliveriesRepository); });
  ...
  afterEach(() => { Object.assign(env.sendgrid, config); jest.restoreAllMocks(); jest.clearAllMocks(); });
  ...
  test("successful email updates shared delivery store visible to admin", async () => {
    ...
    expect(sgMail.send).toHaveBeenCalledWith({ to: payload.to, subject: "Verify", html: payload.html, text: "Verify", from: { email: "sender@example.com", name: "FiFB" } });
    const result = await request(app).get("/api/admin/email-deliveries?limit=1").set("Authorization", auth(actor)).expect(200);
    expect(result.body.data.items[0]).toMatchObject({ recipient_email: payload.to, status: "sent", provider_message_id: "provider-1", template_key: payload.templateKey, sent_at: expect.any(String) });
  });
  test("provider failure records failed status and rethrows", async () => {
    ...
    expect(repository.listDeliveries()[0]).toMatchObject({ status: "failed", error_message: JSON.stringify(error.response.body) });
  });
  test("missing configuration logs failure without contacting provider", async () => {
    ...
    expect(repository.listDeliveries()[0]).toMatchObject({ status: "failed", error_message: "SendGrid configuration is incomplete" });
  });
