...
describe("Nest workout plans with real repository and transaction handling", () => {
  ...
  beforeEach(() => {
    queries = []; owned = true; active = true;
    ...
    pool.connect.mockResolvedValue(client); pool.query.mockImplementation(query);
  });
  ...
  test("create parses defaults, enforces active exercises and commits items with plan", async () => {
    const response = await request(app).post("/api/workout-plans").set("Authorization", auth(actor)).send({ title: " Push ", items: [{ exerciseId: id, restSeconds: 0 }] }).expect(201);
    expect(response.body).toMatchObject({ success: true, message: "Workout plan created", data: { plan: { id, items: [] } } });
    expect(queries[0][0]).toBe("BEGIN"); expect(queries.at(-1)[0]).toBe("COMMIT");
    expect(queries.find(([sql]) => sql.startsWith("INSERT INTO workout_plans"))[1]).toEqual([userId, "Push", null, "private"]);
    expect(queries.find(([sql]) => sql.startsWith("INSERT INTO workout_plan_items"))[1]).toEqual([id, id, 1, 1, null, null, null, 0, null]);
  ...
  test.each(["inactive", "duplicate"])("%s exercise items roll back the whole transaction", async (state) => {
    ...
    await request(app).post("/api/workout-plans").set("Authorization", auth(actor)).send({ title: "Push", items: state === "duplicate" ? [item, item] : [item] }).expect(state === "duplicate" ? 409 : 400);
  ...
  test("list is scoped to owner and archive retains 200 response", async () => {
    ...
    const response = await request(app).delete(`/api/workout-plans/${id}`).set("Authorization", auth(actor)).expect(200);
  ...
  test("updating items uses one transaction and allows clearing all items", async () => {
    await request(app).patch(`/api/workout-plans/${id}`).set("Authorization", auth(actor)).send({ items: [] }).expect(200);
  ...
  test.each(["get", "patch", "delete"])("%s rejects a plan owned by another user", async (method) => {
    owned = false;
    await request(app)[method](`/api/workout-plans/${id}`).set("Authorization", auth(actor)).send(method === "patch" ? { title: "Changed" } : undefined).expect(404);
