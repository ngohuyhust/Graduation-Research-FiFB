...
for (const kind of ["bodyParts", "equipments", "muscles"]) {
  router.get(`/${kind}`, asyncHandler(controller.list(kind)));
  router.post(`/${kind}`, authenticate, requireActiveUser, requireVerifiedEmail, requireRoles("admin"), validate(validation.taxonomySchema), asyncHandler(controller.create(kind)));
}
