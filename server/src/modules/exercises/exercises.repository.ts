...
import type { ExercisePayload, ExerciseUpdate, ExerciseQuery } from "./exercises.validation";
import type { ExerciseRow } from "./exercises.types";


export function pageOffset({ page, limit }: ExerciseQuery) {
  return (page - 1) * limit;
}

export function exerciseLibrarySelect(alias = "e") {
  return `
    ${alias}.*,
...
export function camelExercise(row: ExerciseRow | null | undefined) {
  if (!row) return null;
  return {
    id: row.id,
...
export function addTaxonomyFilter(values: unknown[], clauses: string[], filters: ExerciseQuery, table: string, nameColumn: { table: string; fk: string }, alias: keyof ExerciseQuery, role?: string) {
...
export class ExercisesRepository {
  ...
  async list(filters: ExerciseQuery, admin = false) {
    ...
    const count = await this.db.query<{ total: number }>(`SELECT count(*)::int AS total FROM exercises e WHERE ${where}`, values);
  ...
  async findById(client: QueryExecutor, id: string) {
    const result = await client.query<ExerciseRow>("SELECT * FROM exercises WHERE id = $1", [id]);
    return result.rows[0] || null;
  }

  async ensureActive(client: QueryExecutor, id: string) {
    const result = await client.query<{ id: string }>("SELECT id FROM exercises WHERE id = $1 AND status = 'active'", [id]);
    return Boolean(result.rows[0]);
  }

  async create(client: QueryExecutor, actor: Actor, payload: ExercisePayload, source: string) {
  ...
  async update(client: QueryExecutor, id: string, payload: ExerciseUpdate) {
    const result = await client.query<ExerciseRow>(
      `UPDATE exercises
  ...
  async setReviewStatus(client: QueryExecutor, id: string, actor: Actor, status: string, rejectionReason?: string) {
    const result = await client.query<ExerciseRow>(
      `UPDATE exercises
       SET status = $2,
           reviewed_by = $3,
  ...
  async replaceMappings(client: QueryExecutor, exerciseId: string, payload: ExerciseUpdate) {
