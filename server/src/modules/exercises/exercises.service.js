...
async function updateExercise(actor, id, payload) {
  const exercise = await withTransaction(async (client) => {
    ...
    await repository.replaceMappings(client, id, payload);
...
async function reviewExercise(actor, id, decision) {
  const exercise = await withTransaction(async (client) => {
    const oldExercise = await repository.findById(client, id);
    if (!oldExercise) throw new AppError(codes.NOT_FOUND, "Exercise not found", 404);
    ...
    await auditRepository.createAudit(client, { actorId: actor.userId, action: `exercise.${status}`, entityType: "exercise", entityId: id, oldValues: oldExercise, newValues: updated });
    return updated;
  });
  return { exercise };
}
...
module.exports = { listExercises, getActiveExercise, createExercise, updateExercise, reviewExercise, deactivateExercise };
