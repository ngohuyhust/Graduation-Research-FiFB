...
async function insertItem(client, planId, item) {
  await client.query(
    ...
    [planId, item.exerciseId, item.dayNumber, item.sortOrder, item.sets || null, item.reps || null, item.durationSeconds || null, item.restSeconds ?? null, item.note || null],
  );
}
...
async function getPlan(client, userId, id) {
  ...
  const items = await client.query("SELECT * FROM workout_plan_items WHERE workout_plan_id = $1 ORDER BY day_number ASC, sort_order ASC", [id]);
...
async function list(userId, filters) {
  ...
  const result = await query("SELECT * FROM workout_plans WHERE owner_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3", [userId, filters.limit, pageOffset(filters)]);
