// Service chua nghiep vu chinh cua module trainerConnections.
...
async function sendRequest(userId, payload) {
  const request = await withTransaction(async (client) => {
    ...
    if (!(await repository.findTrainerProfile(client, payload.trainerId)))
      throw new AppError(codes.NOT_FOUND, "Trainer not found", 404);
    if (await repository.findPending(client, userId, payload.trainerId))
      throw new AppError(codes.CONFLICT, "A pending trainer request already exists", 409);
    if (await repository.findActiveConnection(client, userId, payload.trainerId))
...
async function decide(actor, id, status, rejectReason) {
  const request = await withTransaction(async (client) => {
    ...
    if (status === "approved") {
      if (await repository.findActiveConnection(client, pending.user_id, actor.userId))
        throw new AppError(codes.CONFLICT, "An active trainer connection already exists", 409);
    }
